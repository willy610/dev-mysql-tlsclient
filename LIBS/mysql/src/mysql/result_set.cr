...
class MySql::ResultSet < DB::ResultSet
  ...
  def initialize(statement, column_count)
    ...
    @conn.read_column_definitions(columns, column_count, responce_reader: @conn.responce_reader)
  ...
  def move_next : Bool
    ...
    if row_packet = @row_packet
      ...
      @conn.responce_reader.next("MySql::ResultSet move_next()")
      @row_packet = row_packet = @conn.responce_reader.the_message_as_io
# ????
      # @header = @xxx.read_byte!
      # if @header == 0xfe # EOF
      #   @eof_reached = true
      #   return false
      # end
      ...
      # # ??????!!!!!fixa detta
      # puts "fixa detta"
      # # @xxx.read_fully(@null_bitmap_slice)
      # return true
