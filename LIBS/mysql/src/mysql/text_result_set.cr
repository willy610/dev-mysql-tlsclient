...
# https://dev.mysql.com/doc/dev/mysql-server/8.4.6/page_protocol_com_query_response_text_resultset.html
class MySql::TextResultSet < DB::ResultSet
  getter columns

  @conn : MySql::Connection
  @row_packet : Unmarshaller?
  @first_byte : UInt8
  ...
  def initialize(statement, column_count)
    ...
    xxx = SQLResponceReader.new(@conn)
    @conn.read_column_definitions(columns, column_count, responce_reader: xxx)
  ...
  def move_next : Bool
    ...
    #   row_packet.discard
    # end
    @conn.responce_reader.next("MySql::TextSet move_next()")
    @row_packet = row_packet = @conn.responce_reader.the_message_as_io
